...
{movielens website} about the data the following further information is 
available: 
...
The code for this figure is shown
\hyperlink{code_sparsity_figure}{\greenuline{here}}.
...
The code for this chart can be found \hyperlink{code_ratings_by_movie_figure}{\greenuline{here}}.
...
The code for this histogram is displayed \hyperlink{code_ratings_distribution_figure}{\greenuline{here}}.

### Movie Effect {#data_movie_effect}
From public movie ratings such as
\blueref{https://www.rottentomatoes.com/}{Rotten Tomatoes} it is known that some
movies are in general better rated than others. We group the ratings by movie ID
to evaluate the same effect. In addition to the average rating for each movie,
...
higher or lower ratings, compared to other users. We examine this effect by
grouping the ratings by user and arranging them by their average, along
with the standard deviation of the same grouping by user:
...
For example, the parameter $\lambda$ of the regularisation method introduced in
\greenref{method_regularisation} is applied to the
...
and the residual value is $\varepsilon_{u,i}$. As shown in 
...
In addition to simply combining the effects, the parameter $\lambda$ is tuned
...
1. Repeat for $l = 1, 2, \ldots, k = 25$:
    ...
    $\hat b_u(\lambda)$ on the $l$-th training set.
    b) Calculate estimated ratings $\hat y_{u,i}(\lambda)$ on the $l$-th
    validation set.
    c) Use estimated ratings to compute $RMSE_l(\lambda)$ on the $l$-th 
    validation set.
...
(which is confusingly named `validation`) is **not** used in this  calculation
...
`RMSE_data_mur.Rdata` generated by the R script that accompanies this report, as
re-running the tuning takes several hours:

```{r load reg movie user RMSE}
...
report and loaded from the file `train_loess.Rdata`:

```{r load time effect model}
load(file = "data//train_loess.Rdata")
```

The optimal span parameters is $span_{opt} =$  `r train_loess$bestTune$span`
...
\greenref{model_reg_movie_user}, with the optimal regularisation parameter
$\lambda_{mur} =$  `r lambda_mur_opt` for $\hat b_i$ and $\hat b_u$. In
addition, the genre effects $\beta_k$ are estimated as with a separate
regularisation parameter $\lambda_g$ as
\[
...
RMSE_rmu_g_kfold <- function(data, ind_list, lambda_mur, lambda_g) {
    ...
    rmse_v <- sapply(1:k, function (listIdx) {
        ...
        # modify validation set so all moviess, users and genres from training
        # set are contained in it
        validation_set <- validation_set %>%
            semi_join(train_set, by = "movieId") %>%
            semi_join(train_set, by = "userId") %>%
...
observed in \greenref{data_user_effect}.

Combining the movie and user effects has a large incremental effect. That
...
performance, but does result in the **best-performing model** with an RMSE of
0.8648.
