...
For example, the parameter $\lambda$ of the regularisation method introduced
in \greenref{method_regularisation} is applied to the
\colorbox{blizzardblue}{training data} and the RMSE that results from that
particular values of $\lambda$ is evaluated on the 
\colorbox{blond}{test data}. This is done $k=25$ times to produce an estimate
of the RMSE depending on $\lambda$.
